...
def getNews(topics=['ukraine', 'russia', 'war', 'putin', 'zelensky'], limit="50"):
    for topic in topics:
        ...
        querystring = {"q":topic, "pageNumber":"1", "pageSize":limit, "autoCorrect":"true", "withThumbnails":"true","fromPublishedDate":"null","toPublishedDate":"null"}
        headers = {
        ...
        for article in articles['value']:
            if article['image']["url"]:
                ...
                publishedAt = datetime.fromisoformat(article['datePublished'].split('T')[0] + '+00:00')
                ...
                if not NewsArticle.objects.filter(url=url) and not NewsArticle.objects.filter(title=title):
                    new_article = NewsArticle(title=title, description=description, url=url, urlToImage=urlToImage, publishedAt=publishedAt)
                    new_article.save()
                    total += 1
